...
from pisama_n8n_engine.detect.runtime import recorded_timeout
...
class N8NTimeoutDetector(TurnAwareDetector):
    ...
    def _detect_node_timeout(
        ...
        for turn in turns:
            ...
            configured_timeout = turn.turn_metadata.get("configured_timeout_ms")
            is_recorded_timeout = recorded_timeout(turn)
            if is_recorded_timeout:
                threshold = int(configured_timeout)
            if is_recorded_timeout or execution_time_ms > threshold:
                timeout_nodes.append(
                    {
                        ...
                        "recorded_timeout": is_recorded_timeout,
                    }
                )
    ...
    def detect_workflow(
        ...
        for node in nodes:
            ...
            if node_type not in self._WEBHOOK_NODE_TYPES:
                ...
                if (
                    "trigger" not in node_type.lower()
                    and node_type not in self._WEBHOOK_NODE_TYPES
                ):
                    continue
        ...
        for node in nodes:
            ...
            options = params.get("options", {}) or {}
            has_timeout = (
                params.get("timeout") is not None or options.get("timeout") is not None
            )
            if not has_timeout:
        ...
        if not issues:
            return TurnAwareDetectionResult(
                ...
                # returns 0.0 here). The previous 0.9 ranked clean workflows
                # ABOVE the now-advisory positive findings (<=0.45), which would
                # make the calibration threshold-optimizer degenerate.
                severity=TurnAwareSeverity.NONE,
                confidence=0.1,
                failure_mode=None,
        ...
        # Surfacing them as high-confidence SEVERE failures made n8n_timeout
        ...
        # advisory positives still rank ABOVE clean negatives — required for the
        # calibration threshold-optimizer to stay well-defined under this cap.
        severity = TurnAwareSeverity.MINOR
