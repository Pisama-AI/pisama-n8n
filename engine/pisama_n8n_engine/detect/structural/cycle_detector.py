...
class N8NCycleDetector(TurnAwareDetector):
    ...
    def _node_has_iteration_bound(node: Dict[str, Any]) -> bool:
        ...
        (``maxTries`` / ``limit`` on some node versions). This is what a
        ``circuit_breaker`` / ``loop_breaker`` fix sets, so recognizing it here is
        what makes the heal config-re-detect verify faithful: a bounded loop is no
        longer scored ``potentially_infinite``. A genuinely unbounded cycle (no cap
        and no exit branch) still fires — recall on real infinite loops is unchanged.
        """
        params = node.get("parameters") or {}
        for key in ("maxIterations", "maxTries", "limit"):
            val = params.get(key)
